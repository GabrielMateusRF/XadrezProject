public class XequeTeste {
    ...
    public int testeXeque(Posicao[][] mapa, int turno, int mov, int xin, int yin, int xout, int yout, int reix, int reiy){
        val=0;
        if(turno==0){
            inimigo=1;
        }else{
        ...
        ///xequecheck
        ///Caso um movimento que tu fizer te coloca em xeque;
        if (mapa[reix][reiy].getPeca() instanceof Rei && mapa[reix][reiy].getOcupado()==1 && mapa[reix][reiy].getPeca().getCor()==turno){
            System.out.printf("\n i: "+ reix + " j: " + reiy+ "\n");
            for(i2=0; i2<8;i2++){
                for(j2=0;j2<8;j2++) {
                    if(mapa[i2][j2].getOcupado()==1 && mapa[i2][j2].getPeca().getCor()==inimigo){
                        ...
                        if (mapa[i2][j2].getPeca() instanceof Peao) {
                            System.out.printf("\n P \n");
                            System.out.printf("\n K1 \n");
                            val=((Peao) mapa[i2][j2].getPeca()).movValido(mapa, reix, reiy, i2, j2);
                            System.out.printf("\n"+ val +"Cor: "+ mapa[i2][j2].getPeca().getCor()  +"\n");
                        } else {
                            if (mapa[i2][j2].getPeca() instanceof Torre) {
                                System.out.printf("\n K2 \n");
                                if(mapa[reix][reiy].getPeca() instanceof Rei){
                                    if(mapa[reix][reiy].getPeca().getCor()==0){
                                        System.out.printf("\n REI BRANCO\n");
                                    }
                                }
                                if(mapa[i2][j2].getPeca() instanceof Torre){
                                ...
                                val=((Torre) mapa[i2][j2].getPeca()).movValido(mapa, reix, reiy, i2, j2);
                                System.out.printf("\n"+ val +"Cor: "+ mapa[i2][j2].getPeca().getCor() +"\n");
                            } else {
                                if (mapa[i2][j2].getPeca() instanceof Cavalo) {
                                    System.out.printf("\n K3 \n");
                                    val=((Cavalo) mapa[i2][j2].getPeca()).movValido(mapa, reix, reiy, i2, j2);
                                    System.out.printf("\n"+ val +"Cor: "+ mapa[i2][j2].getPeca().getCor() +"\n");
                                } else {
                                    if (mapa[i2][j2].getPeca() instanceof Bispo) {
                                        System.out.printf("\n K4 \n");
                                        val=((Bispo) mapa[i2][j2].getPeca()).movValido(mapa, reix, reiy, i2, j2);
                                        System.out.printf("\n"+ val +" Cor: "+ mapa[i2][j2].getPeca().getCor() +"\n");
                                    } else {
                                        if (mapa[i2][j2].getPeca() instanceof Rainha) {
                                            System.out.printf("\n K5 \n");
                                            val=((Rainha) mapa[i2][j2].getPeca()).movValido(mapa, reix, reiy, i2, j2);
                                        }else{
                                            if (mapa[i2][j2].getPeca() instanceof Rei) {
                                                val=0;
                                            }
